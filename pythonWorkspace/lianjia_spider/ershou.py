...
def get_area_ershou_info(city_name, area_name):
    ...
    for num in range(1, total_page + 1):
        ...
        for house_elem in house_elements:
            ...
            # 房屋编号
            matches = re.search('.*data-housecode="(\d+)', str(name))
            if matches is not None:
                house_code = matches.group(1)
            else:
                matches = re.search('.*data-lj_action_housedel_id="(\d+)', str(name))
                if matches is not None:
                    house_code = matches.group(1)
                else:
                    house_code = 0
            ...
            house_code=house_code.strip()

            # 作为对象保存
            ershou = ErShou(chinese_district, chinese_area, name, price, desc, house_code)
