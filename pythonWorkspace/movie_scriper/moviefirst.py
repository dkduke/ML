...
def download_mp4(mp4_url,path):
    ...
    #.decode('utf-8').encode('gbk')是utf-8到unicoode再到gbk
    #path="/Users/dengkun/Downloads/budujie_mv/{}.mp4".format(path)
    path = "/Users/dengkun/Downloads/mv/{}.mp4".format(path)
    if not os.path.exists(path):
        urllib.request.urlretrieve(mp4_url,path)#下载，方法一
        print('ok!!!')
        # 方法二  （三行）
...
if __name__=='__main__' :#判断是不是当前文件执行
    #全局变量
    #http://www.meipai.com/search/mv?q=%E9%AB%98%E9%A2%9C%E5%80%BC&page=30
    start_urls=['http://www.meipai.com/search/mv?q=%E9%AB%98%E9%A2%9C%E5%80%BC&page={}'.format(i) for i in range(10,20)]
    #start_urls=['http://www.meipai.com/square/474']
    print(start_urls)
    main()
